...
i2c_master_dev_handle_t axp2101_i2c_dev_hd = NULL;
...
static void task_power_management(void *param)
{
    ...
    while (1){ if (xQueueReceive(axp2101_evt_queue, &io_num,portMAX_DELAY)) {//wait for interrupt
        if (io_num==IOPIN_PMIC_PWR){
            i2c_master_transmit_receive(axp2101_i2c_dev_hd, &address, 1,data, sizeof(data),1000);
...
void AXP2101_init(void)
{
    ...
    ESP_ERROR_CHECK(i2c_master_bus_add_device(i2c_handle, &axp2101_config, &axp2101_i2c_dev_hd));
    i2c_master_transmit(axp2101_i2c_dev_hd, &data[0], 2,100);
    i2c_master_transmit(axp2101_i2c_dev_hd, &data[2], 4,100);
    i2c_master_transmit_receive(axp2101_i2c_dev_hd, &address, 1,data, sizeof(data),500);
...
esp_err_t AXP2101_bat_percentage(void)
{
    ...
    i2c_master_transmit_receive(axp2101_i2c_dev_hd, &data, 1, &data, sizeof(data),100);
    return data;
}
...
void AXP2101_IRQStatus_clear(void)
{
    ...
    i2c_master_transmit(axp2101_i2c_dev_hd, &data[0], sizeof(data),100);
}
